...
case class PublicDatasets(alias: String, tableDatabase: Option[String], viewDatabase: Option[String]){
  ...
  val sources: List[DatasetConf] = List(
          ...
          DatasetConf("raw_ensembl_canonical"        , alias, "/raw/landing/ensembl/Homo_sapiens.GRCh38.canonical.tsv.gz"          , CSV  , OverWrite , readoptions = Map("header" -> "false", "sep" -> "\t")),
          ...
          DatasetConf("enriched_rare_variant"              , alias, "/public/rare_variant/enriched", DELTA, OverWrite , partitionby = List("chromosome"), table = table("rare_variant_enriched"), view = view("rare_variant_enriched"))
