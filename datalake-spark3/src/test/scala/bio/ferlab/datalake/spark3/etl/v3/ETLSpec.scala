...
class ETLSpec extends WithETL {
  ...
  case class TestETL(rc: DeprecatedRuntimeETLContext) extends SimpleETL(rc) {
  ...
  override val defaultJob: SimpleETL = TestETL(DeprecatedTestETLContext())
  ...
  "skip" should "not run the etl" in {
    val job = TestETL(DeprecatedTestETLContext(RunStep.getSteps("skip")))
