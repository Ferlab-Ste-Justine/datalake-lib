...
import bio.ferlab.datalake.testutils.{CleanUpBeforeEach, TestETLContext}
...
class SingleETLSpec extends WithETL with CleanUpBeforeEach {

  import spark.implicits._

  case class SingleTestETL(rc: RuntimeETLContext) extends SimpleSingleETL(rc) {
  ...
  override val defaultJob: ETL[LocalDateTime, SimpleConfiguration] = SingleTestETL(TestETLContext())
  ...
  "skip" should "not run the etl" in {
    val job = SingleTestETL(TestETLContext(RunStep.getSteps("skip")))
  ...
  "initial_load" should "run the ETL as if it were the first time running" in {
    ...
    val job = SingleTestETL(TestETLContext(RunStep.initial_load))
  ...
  "sample_load" should "run the ETL with a sampled data and override the current table" in {
    ...
    val job = SingleTestETL(TestETLContext(RunStep.allSteps))
    job.load(Map(destConf.id -> Seq(AirportOutput(11)).toDF()))
